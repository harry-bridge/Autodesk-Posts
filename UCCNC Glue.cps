...
function onOpen() {
  ...
  //Dump movement codes
  // writeComment("Movement types:");
  // for (var i = 0; i < movementNumbers.length; i++) {
  //   var _movString = movementNames[i] + " - " + movementNumbers[i];
  //   writeComment(_movString);
  // }
...
function onMovement(movement) {
  var _delayAfterOn = 80;
  var _delayBeforeOff = 30;
  var _delayAfterOff = 300;

  var _movString = "undefined";
  for (var i = 0; i < movementNumbers.length; i++) {
    if (movementNumbers[i] == movement) {
      _movString = movementNames[i] + " - " + movementNumbers[i];
      break;
    }
  }
  writeComment(_movString);
  ...
  if (movement == MOVEMENT_CUTTING || movement == MOVEMENT_FINISH_CUTTING) {
    ...
    writeBlock(gFormat.format(4), pFormat.format(_delayAfterOn));
  } else if (movement == MOVEMENT_LEAD_OUT || movement == MOVEMENT_RAPID) {
    writeBlock(gFormat.format(4), pFormat.format(_delayBeforeOff));
    writeBlock(mFormat.format(11));
    writeBlock(gFormat.format(4), pFormat.format(_delayAfterOff));
    writeBlock(gFormat.format(0), zOutput.format(-.2));
    writeBlock(gFormat.format(4), pFormat.format(30));
  } else {
    writeBlock(mFormat.format(11));
  }
  ...
  //   case MOVEMENT_RAPID:
  //     writeBlock(mFormat.format(11));
  //     writeBlock(gFormat.format(4), pFormat.format(100));
  //     break;
  //   default:
  //     break;
  // }
